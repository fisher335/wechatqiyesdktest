...
class WeChatEnterprise(object):
    ...
    def get_access_token(self):
        ...
        s = Token_cache.getvalue()
        d = json.loads(s)
        access_token = d["token"]
        if access_token == "" or d['time'] >= time.time():
    ...
    def create_department(self, name, parentid=1, order=None):
        ...
        """
        url = "%s/department/create?access_token=%s" % (self.url_prefix, self.get_access_token())
        data = {
            "name": name,
            "parentid": parentid,
        }
    ...
    def update_department(self, department_id, **kwargs):
        ...
        """
        url = "%s/department/update?access_token=%s" % (self.url_prefix, self.get_access_token)
        data = {
            "id": department_id,
        }
        data.update(kwargs)
    ...
    def delete_department(self, department_id):
        ...
        """
        url = "%s/department/delete?access_token=%s&id=%s" % (self.url_prefix, self.get_access_token(), department_id)
    ...
    def get_department_list(self):
        """
            ...
            access_token	是	调用接口凭证
        """
        url = "%s/department/list?access_token=%s" % (self.url_prefix, self.get_access_token())
        status, res = self.__get(url)
        return status, res
    ...
    def create_user(self, data):
        ...
        """
        url = "%s/user/create?access_token=%s" % (self.url_prefix, self.get_access_token())
    ...
    def update_user(self, userid, **kwargs):
        """
            ...
            extattr	否	扩展属性。扩展属性需要在WEB管理端创建后才生效，否则忽略未知属性的赋值
        """
        url = "%s/user/update?access_token=%s" % (self.url_prefix, self.get_access_token())
        data = {"userid": userid}
        data.update(kwargs)
    ...
    def delete_user(self, userid):
        ...
        """
        url = "%s/user/delete?access_token=%s&userid=%s" % (self.url_prefix, self.get_access_token(), userid)
        status, res = self.__get(url)
        return status, res
    ...
    def multi_delete_user(self, useridlist):
        ...
        """
        url = "%s/user/batchdelete?access_token=%s" % (self.url_prefix, self.get_access_token())
    ...
    def get_user(self, userid):
        ...
        """
        url = "%s/user/get?access_token=%s&userid=%s" % (self.url_prefix, self.get_access_token(), userid)
        status, res = self.__get(url)
        return status, res
    ...
    def get_users_in_department(self, department_id, fetch_child=0, status=0):
        ...
        url = "%s/user/simplelist?access_token=%s&department_id=%s&fetch_child=%s&status=%s" \
              % (self.url_prefix, self.get_access_token(), department_id, fetch_child, status)
        status, res = self.__get(url)
        return status, res
    ...
    def invite_attention_to_user(self, userid, invite_tips=None):
        ...
        """
        url = "%s/invite/send?access_token=%s" % (self.url_prefix, self.get_access_token())
        data = {
            "userid": userid
        }
    ...
    def create_tag(self, tagname):
        ...
        """
        url = "%s/tag/create?access_token=%s" % (self.url_prefix, self.get_access_token())
    ...
    def update_tag(self, tagid, tagname):
        """
            ...
            tagname	是	标签名称。长度为1~64个字符，标签不可与其他同组的标签重名，也不可与全局标签重名
        """
        url = "%s/tag/update?access_token=%s" % (self.url_prefix, self.get_access_token())
    ...
    def delete_tag(self, tagid):
        """
            ...
            tagid	是	标签ID
        """
        url = "%s/tag/delete?access_token=%s&tagid=%s" % (self.url_prefix, self.get_access_token(), tagid)
        status, res = self.__get(url)
        return status, res
    ...
    def get_user_from_tag(self, tagid):
        """
            ...
            tagid	是	标签ID
        """
        url = "%s/tag/get?access_token=%s&tagid=%s" % (self.url_prefix, self.get_access_token(), tagid)
        status, res = self.__get(url)
        return status, res
    ...
    def add_users_to_tag(self, tagid, userlist, partylist):
        ...
        """
        url = "%s/tag/addtagusers?access_token=%s" % (self.url_prefix, self.get_access_token())
    ...
    def delete_user_in_tag(self, tagid, userlist, partylist):
        """
            ...
            partylist	否	企业部门ID列表，注意：userlist、partylist不能同时为空
        """
        url = "%s/tag/deltagusers?access_token=%s" % (self.url_prefix, self.get_access_token())
        data = {"tagid": tagid, "userlist": userlist, "partylist": partylist}
        status, res = self.__post(url, data=data)
        return status, res
    ...
    def get_tag_list(self):
        """
            ...
            access_token	是	调用接口凭证
        """
        url = "%s/tag/list?access_token=%s" % (self.url_prefix, self.get_access_token())
        status, res = self.__get(url)
        return status, res
    ...
    def upload_media(self, media_type, media_file):
        ...
        """
        url = "%s/media/upload?access_token=%s&type=%s" % (self.url_prefix, self.get_access_token(), media_type)
    ...
    def get_media(self, media_id):
        """
            ...
            media_id	是	媒体文件id
        """
        url = "%s/media/get?access_token=%s&media_id=%s" % (self.url_prefix, self.get_access_token(), media_id)
    ...
    def send_msg_to_user(self, content, touser=None, toparty=None, totag=None, safe=0, msgtype="text", **kwargs):
        ...
        """
        url = "%s/message/send?access_token=%s" % (self.url_prefix, self.get_access_token())
        data = {
            "safe": safe,
            "msgtype": msgtype,
